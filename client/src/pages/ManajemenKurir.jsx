...
const ManajemenKurir = () => {
  ...
  useEffect(() => {
    const fetchKurir = async () => {
      try {
        ...
        console.error("Gagal mengambil data kurir dari backend:", err);
      }
    };
    fetchKurir();
  }, []);
  ...
  const filteredKurirs = kurirs.filter((kurir) => {
    ...
    // 3. Logika pencarian dropdown status
        ...
        <div style={kurirGrid}>
          {filteredKurirs.length > 0 ? (
            filteredKurirs.map((kurir) => {
              ...
              return (
                <div key={kurir._id} style={kurirCard}>
                  ...
                  <div style={cardBody}>
                    <p>Tugas Aktif: <span style={{color: 'white'}}>{kurir.tugasAktif || 0}</span> Bag</p>
            ...
            <p style={{ color: '#9ca3af', fontSize: '14px', gridColumn: '1 / -1' }}>
              Tidak ada data kurir terdaftar yang cocok dengan pencarian Anda.
            </p>
          )}
        </div>
      </main>
...
const kurirCard = { backgroundColor: '#1f2937', padding: '20px', borderRadius: '15px', border: '1px solid #374151', transition: 'transform 0.2s', ':hover': { transform: 'translateY(-2px)' } };
...
const progressFill = { height: '100%', backgroundColor: '#3b82f6', transition: 'all 0.3s' };
